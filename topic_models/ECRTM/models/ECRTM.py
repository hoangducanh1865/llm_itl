...
class ECRTM(nn.Module):
    def __init__(self, args):
        ...
        self.a = 1 * np.ones((1, args.n_topic)).astype(np.float32)
        self.mu2 = nn.Parameter(torch.as_tensor((np.log(self.a).T - np.mean(np.log(self.a), 1)).T))
        self.var2 = nn.Parameter(torch.as_tensor((((1.0 / self.a) * (1 - (2.0 / args.n_topic))).T + (1.0 / (args.n_topic * args.n_topic)) * np.sum(1.0 / self.a, 1)).T))
        ...
        self.fc21 = nn.Linear(args.en1_units, args.n_topic)
        self.fc22 = nn.Linear(args.en1_units, args.n_topic)
        ...
        self.topic_embeddings = torch.empty((args.n_topic, self.word_embeddings.shape[1]))
    ...
    def compute_loss_KL(self, mu, logvar):
        ...
        # KLD: N*K
        KLD = 0.5 * ((var_division + diff_term + logvar_division).sum(axis=1) - self.args.n_topic)
        KLD = KLD.mean()
        return KLD
